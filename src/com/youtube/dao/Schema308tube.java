	...
	public JSONArray queryReturnBrandParts(String brand) throws Exception {
		...
		try {
			...
			query = conn.prepareStatement("select PC_PARTS_PK, PC_PARTS_TITLE, PC_PARTS_CODE, "
						+"PC_PARTS_MAKER, PC_PARTS_AVAIL, PC_PARTS_DESC from PC_PARTS " +
											"where UPPER(PC_PARTS_MAKER) = ? ");

			query.setString(1, brand.toUpperCase()); //protect against sql injection
			// это типа безопасный запрос именно поэтому выше стоит в конце запроса вопросительный знак ?  см. Part 5 - 15:00
